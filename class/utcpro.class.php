...
class utcPro {
	...
	if ($this->sourceX < $this->width && $this->sourceY < $this->height) {
	    ...
	    $A	 = $this->width;
	    $B	 = $this->sourceY * $this->width / $this->sourceX;
	    $Xc	 = 0;
	    $Yc	 = floor(($this->height - $B) / 2);
	}
	if ($B > $this->height) {
	    $A	 = $this->sourceX * $this->height / $this->sourceY;
	    $B	 = $this->height;
	    $Xc	 = floor(($this->width - $A) / 2);
	    $Yc	 = 0;
	}
	...
	$folders	 = explode('/', $pathData['dirname'] . '/' . $firstChar);

	$currentFolder = '';

	foreach ($folders as $folder) {
	    $currentFolder .= $folder . '/';
	    if (!is_dir($this->cachePath . $currentFolder) && !file_exists($this->cachePath . $currentFolder)) {
		@mkdir($this->cachePath . $currentFolder, 0777);
